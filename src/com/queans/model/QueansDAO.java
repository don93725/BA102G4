...
public class QueansDAO implements QueansDAO_interface {
	...
	static {
		...
		private static final String UPDATE_TWO =
			"UPDATE queans set upd_date=sysdate,que_type=?,que_title=?,ans_ctx=? where que_no = ?";
		private static final String GET_PIC = 
			"select que_photo from queans where que_no=?";
	...
	public void updateTwo(QueansVO queansVO) {
		...
		try {
			...
			pstmt.setString(2, queansVO.getQue_title());
			pstmt.setString(3, queansVO.getAns_ctx());
			pstmt.setString(4, queansVO.getQue_no());
