...
public class Course_listDAO implements Course_listDAO_interface{
	...
	public List<Course_listVO> getReserve(String stu_acc) {
		...
		try {
			...
			String finalSQL = "SELECT * FROM COURSE_LIST CL JOIN COURSE_TIME  CT  ON CL.CT_NO = CT.CT_NO LEFT OUTER JOIN PLACE P  ON CT.P_NO = P.P_NO JOIN STUDENTS STU ON cl.stu_acc = stu.stu_acc left join course_picture cp ON CT.crs_no = cp.crs_no JOIN COURSE CRS ON CT.CRS_NO = crs.crs_no JOIN GYMS G ON  P.G_ACC = G.GYM_ACC WHERE CT.STATUS=1 AND CL.STU_ACC ="
			           +"'"+stu_acc+"'"
			          ;
	...
	public List<Course_listVO> getCoachReady(String coa_acc) {
		...
		try {
			...
			String finalSQL = "SELECT * FROM COURSE_LIST CL JOIN COURSE_TIME  CT  ON CL.CT_NO = CT.CT_NO LEFT OUTER JOIN PLACE P  ON CT.P_NO = P.P_NO JOIN STUDENTS STU ON cl.stu_acc = stu.stu_acc left join course_picture cp ON CT.crs_no = cp.crs_no JOIN COURSE CRS ON CT.CRS_NO = crs.crs_no join coaches coa on CRS.c_acc=coa.coa_acc JOIN GYMS G ON  P.G_ACC = G.GYM_ACC WHERE CT.status = 2 AND STU_PAY_STA=1 AND COA.COA_ACC ="
			           +"'"+coa_acc+"'"
			          ;
	...
	public List<Course_listVO> getCoachFinished(String coa_acc) {
		...
		try {
			...
			String finalSQL = "SELECT * FROM COURSE_LIST CL JOIN COURSE_TIME  CT  ON CL.CT_NO = CT.CT_NO LEFT OUTER JOIN PLACE P  ON CT.P_NO = P.P_NO JOIN STUDENTS STU ON cl.stu_acc = stu.stu_acc  left join course_picture cp ON CT.crs_no = cp.crs_no JOIN COURSE CRS ON CT.CRS_NO = crs.crs_no join coaches coa on CRS.c_acc=coa.coa_acc JOIN GYMS G ON  P.G_ACC = G.GYM_ACC WHERE CT.status = 2 AND CL.n_sta=1  AND COA.COA_ACC ="
			           +"'"+coa_acc+"'"
			          ;
