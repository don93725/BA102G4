...
public class AnnewDAO implements AnnewDAO_interface {
	...
	private static final String UPDATE_TWO =
	    "UPDATE annew set upd_date=sysdate,ann_title=?,ann_ctx=?,att_no=? where ann_no = ?";
	private static final String GET_PIC = 
		"select ann_photo from annew where ann_no=?";		
	...
	public void updateTwo(AnnewVO annewVO) {
		...
		try {
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(UPDATE_TWO);
			
			pstmt.setString(1, annewVO.getAnn_title());
			pstmt.setString(2, annewVO.getAnn_ctx());
			pstmt.setBytes(3, annewVO.getAtt_no());
			pstmt.setString(4, annewVO.getAnn_no());
	...
	public List<AnnewVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				annewVO.setUpd_date(rs.getDate("upd_date"));
				annewVO.setAnn_title(rs.getString("ann_title"));
				String ctx = rs.getString("ann_ctx");
				if(ctx.length()>10){
