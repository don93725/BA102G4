...
public class PlatinfDAO implements PlatinfDAO_interface {
	...
	static {
		...
		private static final String UPDATE_TWO = 
			"UPDATE platinf set com_address=?,cp_no=?,cs_email=?,pr_policy=?,upd_date=sysdate where pin_no = ?";
		private static final String GET_PIC = 
			"select pin_photo from platinf where pin_no=?";
	...
	public void updateTwo(PlatinfVO platinfVO) {
		...
		try {
			...
			pstmt.setString(3, platinfVO.getCs_email());
			pstmt.setString(4, platinfVO.getPr_policy());
			pstmt.setString(5, platinfVO.getPin_no());
